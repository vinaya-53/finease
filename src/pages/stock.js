...
const StockDetailPage = () => {
    ...
    useEffect(() => {
        const fetchStockData = async () => {
            try {
              ...
              // Make API requests
              const priceRes = await axios.get(`https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol=${companySymbol}&apikey=${process.env.Alpha_Vantage_API_KEY}`);
              const overviewRes = await axios.get(`https://www.alphavantage.co/query?function=OVERVIEW&symbol=${companySymbol}&apikey=${process.env.Alpha_Vantage_API_KEY}`);
              ...
              const graphValues = graphLabels.map(date => timeSeries[date]['4. close']); // Get the closing prices
          
              setGraphData({
                labels: graphLabels,
                datasets: [{ label: `${companySymbol} Stock Price`, data: graphValues, borderColor: 'rgba(75,192,192,1)', tension: 0.1 }]
              });
    ...
    const calculateVolatility = (prices) => {
      ...
      prices.forEach(price => { sum += Math.pow(price - mean, 2); });
          ...
          <Col>
            <Dropdown>
              <Dropdown.Toggle variant="success" id="dropdown-basic">
                {companies.find(company => company.symbol === companySymbol)?.name || 'Select a Company'}
              </Dropdown.Toggle>
              <Dropdown.Menu>
                {companies.map(company => (
                  <Dropdown.Item key={company.symbol} onClick={() => setCompanySymbol(company.symbol)}>
                    {company.name}
                  </Dropdown.Item>
                ))}
              </Dropdown.Menu>
          ...
          <Col>
            <Card>
              <Card.Body>
                ...
                <Button variant="info" onClick={handleLearnMoreClick}>Learn More</Button>
              </Card.Body>
            </Card>
          </Col>
        </Row>
